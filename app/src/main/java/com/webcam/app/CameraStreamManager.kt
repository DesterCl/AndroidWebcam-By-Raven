...
class CameraStreamManager(
    ...
    // Características del dispositivo (se leen una sola vez al abrir)
    ...
    fun startCamera(width: Int, height: Int, useFrontCamera: Boolean) {
        ...
        try {
            ...
            // Elegir resolución en YUV_420_888 (único formato garantizado para streaming)
            ...
            Log.d("CAM", "Res=${bestSize.width}x${bestSize.height} sensor=$sensorOrientation")

            // ImageReader con 2 buffers — suficiente y evita presión de memoria
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        ...
        override fun onDisconnected(camera: CameraDevice) {
            Log.w("CAM", "onDisconnected")
            camera.close()
            cameraDevice = null
        }
        override fun onError(camera: CameraDevice, error: Int) {
            // error=1 → ERROR_CAMERA_DEVICE (HAL),  error=2 → ERROR_CAMERA_SERVICE
            // error=3 → MAX_CAMERAS_IN_USE,          error=4 → CAMERA_DISABLED
            // error=5 → ERROR_CAMERA_IN_USE
            val msg = when (error) {
            ...
            camera.close()
            cameraDevice = null
            // Si la cámara está en uso, reintentamos tras una pausa
    ...
    private fun startRepeating(camera: CameraDevice, session: CameraCaptureSession) {
        try {
            captureRequestBuilder = camera.createCaptureRequest(CameraDevice.TEMPLATE_RECORD).also { b ->
                ...
                // ── Región de metering centrada (solo si el dispositivo lo soporta) ──
                ...
                // ── Estabilización solo si el HAL la declara ─────────────
    ...
    private fun chooseFpsRange(): Range<Int> {
        ...
        // Preferir rango con upper=30 y el span más pequeño posible
        return ranges
            .filter { it.upper >= 24 }
    ...
    private fun calculateRotation(): Int =
        if (isFrontCamera) (sensorOrientation + currentRotation + 360) % 360
        else               (sensorOrientation - currentRotation + 360) % 360

    /** Convierte YUV_420_888 → NV21 respetando rowStride/pixelStride. */
    private fun yuv420ToNv21(image: Image): ByteArray {
        ...
        // Canales UV intercalados (NV21 = V primero, luego U)
