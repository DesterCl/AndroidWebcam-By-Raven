...
class CameraStreamManager(
    ...
    // Buffer reutilizable para rotación
    private val rotateBuffer = ByteArrayOutputStream()
    ...
    private fun openById(cameraId: String, width: Int, height: Int) {
        try {
            ...
            // ── CAMBIO CLAVE: usar JPEG directo del HAL ──────────────────────
            // El sensor entrega JPEG comprimido por hardware → sin conversión CPU
            val jpegSizes = map.getOutputSizes(ImageFormat.JPEG)
            val bestSize  = jpegSizes
                .filter { it.width <= width && it.height <= height }
                .maxByOrNull { it.width.toLong() * it.height }
                ?: jpegSizes.minByOrNull { Math.abs(it.width - width) + Math.abs(it.height - height) }
                ?: run { onErrorMessage("Sin resolución compatible"); return }

            Log.d("CAM", "JPEG HAL ${bestSize.width}x${bestSize.height} rot=$cachedRotation fps=${chooseFpsRange()}")
            ...
            Log.e("CAM", "openById: ${e.message}"); onErrorMessage("Error: ${e.message}")
        }
    }
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        override fun onOpened(camera: CameraDevice) {
            ...
            try {
                ...
                Log.e("CAM", "createSession: ${e.message}"); onErrorMessage("Error sesión: ${e.message}")
            }
        }
        override fun onDisconnected(camera: CameraDevice) { camera.close(); cameraDevice = null }
        override fun onError(camera: CameraDevice, error: Int) {
            val msg = when (error) {
                1->"Error interno HAL"; 2->"Error servicio cámara"
                3->"Demasiadas cámaras"; 4->"Cámara deshabilitada"
                5->"Cámara en uso"; else->"Error $error"
            }
        ...
        override fun onConfigured(session: CameraCaptureSession) {
            ...
            try {
                captureRequestBuilder = session.device.createCaptureRequest(CameraDevice.TEMPLATE_PREVIEW).also { b ->
                    ...
                    // Calidad JPEG máxima desde el HAL
                ...
                Log.e("CAM", "startRepeating: ${e.message}"); onErrorMessage("Error captura: ${e.message}")
            }
        }
    ...
    private fun chooseFpsRange(): Range<Int> {
        ...
        // 1. Rango fijo más alto (ej: [60,60] o [30,30])
        val fixed = ranges.filter { it.lower == it.upper && it.upper >= 24 }
            .maxByOrNull { it.upper }
        if (fixed != null) return fixed
        // 2. Rango variable con upper más alto
    ...
    private fun rotateJpeg(jpeg: ByteArray, rotation: Int): ByteArray {
        return try {
            ...
            rotateBuffer.reset()
            rot.compress(Bitmap.CompressFormat.JPEG, 92, rotateBuffer)
            bmp.recycle(); rot.recycle()
            rotateBuffer.toByteArray()
    ...
    fun stopCamera() {
        try {
            ...
            captureRequestBuilder = null; flashEnabled = false
            rotateBuffer.reset()
